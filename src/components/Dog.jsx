...
const Dog = () => {
  ...
  const branchsMaterial = new THREE.MeshMatcapMaterial({
  ...
  model.scene.traverse((child) => {
    if (child.name.includes("DOG")) {
      ...
      child.material = branchsMaterial;
    }
  });
  ...
  useEffect(() => {
    document
      .querySelector(`.title[img-title="tommoroland"]`)
  ...
  return (
    <>
      <primitive
        object={model.scene}
        position={[0.2, -0.57, 0.18]}
        rotation={[0, Math.PI / 6.5, 0]}
      />
      <directionalLight color={0xffffff} position={[0, 5, 5]} />
    </>
  );
};
